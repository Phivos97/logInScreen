  ...
  const handleSignUp = () => {
    if (user.length < 5 || password.length < 5) {
      ...
      return setErrorMessage(
        'Username and password fields must contain at least 5 characters'
      );
    }
    if (JSON.parse(localStorage.getItem('users')).find(isName)) {
      setToggleSuccess(false);
    ...
    <>
      <div className='relative h-screen flex justify-center items-center text-white'>
        ...
        >
          ...
          />
          <Link
            className='text-center hover:text-[#F0B7B3] hover:cursor-pointer brightness-110'
            to='/'
          >
        ...
        <span
          className={`absolute bottom-0 backdrop-blur-3xl opacity-95 brightness-150 w-full font-semibold text-xl border-solid  py-3 px-3 rounded-lg justify-center text-black text-center transition ease-in-out duration-[600ms] ${
            toggleError
              ? 'bg-red-500'
              : toggleSuccess
              ? 'bg-green-500'
